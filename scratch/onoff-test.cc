...
std::string fileNameRoot = "onoff-test";    // base name for trace files, etc
...
main (int argc, char *argv[])
{
  ...
  Ptr<ConstantRandomVariable> const_rand_var = CreateObject<ConstantRandomVariable>();

  const_rand_var->SetAttribute("Constant", DoubleValue(1));

  NS_LOG_UNCOND(const_rand_var->GetValue());
  NS_LOG_UNCOND(const_rand_var->GetValue());
  NS_LOG_UNCOND(const_rand_var->GetValue());
  NS_LOG_UNCOND(const_rand_var->GetValue());
